...
namespace RabbitWings.Core.Editor.AutoFillSettings
{
	...
	public class AutoFillTool
	{
        ...
        private void ApplySelection(int[] selection)
        {
            ...
            {
                ShowError($"Error: selection to apply is empty");
                return;
            }
            ...
            {
                ShowError($"Error: inconsistent selection length");
                return;
            }
            ...
            try
            {
                ...
                var projectID = values[1]?.ToString() ?? string.Empty;
                var loginID = values[2]?.ToString() ?? string.Empty;
                var oauthID = values[3] != null ? Convert.ToInt32(values[3]) : 0;
                var redirectURL = values[4]?.ToString() ?? string.Empty;

#if AUTOFILLTOOL_DEBUG
        XDebug.Error($"Selection applied: [{projectID},{loginID},{oauthID},{redirectURL}]");
#else
                XsollaSettings.StoreProjectId = projectID;
                XsollaSettings.LoginId = loginID;
                XsollaSettings.OAuthClientId = oauthID;
